...
class GroovyEditor extends CodeArea implements Closeable {
	...
	new(File sourceFile) {
		...
		multiPlainChanges().successionEnds(Duration.ofMillis(500)).supplyTask[computeHighlightingAsync()].awaitLatest(
			multiPlainChanges()).filterMap [
			if (isSuccess()) {
